#
# Ruby process for batch import to IUSE Herbarium collection, called by rake tasks
#
# This is used to import a batch for the IUSE Herbarium, using an associated CSV file
# Task is run with 'rake cbrc:iuse_batch_import:iuse_batch_import[filename.csv,email@example.com,"NO"]
...
module Cbrc
  ...
  module IuseBatchImport

    module Tasks

      def Tasks.iuse_batch_import(data_file, owner_username, deleteafteringest)
        print "------\n"
          ...
          CSV.foreach(data_file, headers:true) do |row|
            ...
            if gfs.size == 0
              ...
              gf = Work.create!(multivalue_row.to_h) do |obj|
                ...
                obj.collection_code = ['iuse']
                obj.identifier = ["http://purl.dlib.indiana.edu/iudl/VAD9213/VAD9213-#{cat_num}"]
              ...
              gf.collection_code = ['iuse']
              gf.identifier = ["http://purl.dlib.indiana.edu/iudl/VAD9213/VAD9213-#{cat_num}"]
...
From: FROMEMAIL
To: TOEMAIL
Subject: IUSE Imago Ingest Complete
